document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("/flyday/pkgplanpic/select", {
        method: "POST",
        ...
        <div class="card p-2 mb-0 shadow-none border position-relative h-100">
            <!-- Image -->
            <img data-dz-thumbnail src="${data.pkgPlanImg}" class="rounded bg-light" alt="">
                <!-- info -->
                <div class="mt-2">
                ...
                planpic.push(data.pkgPlanPicNo);
            });
    ...
    bt.addEventListener("click", async function () {
        await Promise.all(
            planpic.map(async (planno) => {
                await fetch("/flyday/pkgplanpic/remove", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        pkgPlanPicNo: planno
